...
Writing
	...
	Additional header fields are required for specific formats
		...
		PFM requires `scale` and `endian`, though the latter may be left as default (Big)
...
Some syntax differences from the specifications:
	...
	`scale` and `endian` are separated, so the `header` will always store a positive `scale`
	`endian` will only be `Little` for a negative `scale` PFM, every other format will be `Big`
	`endian` only describes the netpbm data being read/written, the image buffer will be native
*/
